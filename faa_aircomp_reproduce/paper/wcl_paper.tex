...
where the access point repositions $N$ ports within a 30\,cm aperture. A block coordinate descent (BCD) algorithm jointly
...
scheme saves up to 47\% transmit energy over a fixed-array
...
on the same time--frequency resource, and the AP recovers the desired
...
battery-powered IoT devices participating in thousands of FL rounds,
...
$p_{k}\!\in\![0,P_{\max}]$ and $|\tau_{k}|\!\leq\!1$. After receive
...
compressing the feasible set super-linearly. The sub-linear exponent on
$N$ ($0.28\!<\!1$) reflects diminishing returns: the first few ports
...
while doubling $N$ reduces it by only $2^{0.28}\approx 1.21\times$---so
...
$E^{*}\approx 0.244\cdot 8^{1.34}\cdot 6^{0.28}\approx 3.95$\,W$\cdot$sym,
...
noise floor ($\approx\!0.04$), reaching 47\% energy reduction at
...
rank-limited). The weak $N$ exponent ($0.28$) in~\eqref{eq:scaling}
...
\subsection{Straggler Robustness and APV Ablation}
...
closed-form planning. At 5\,GHz, up to 47\% energy is saved with six
